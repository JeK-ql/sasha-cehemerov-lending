...
public/
  hero.webp front.webp back.webp artist-front.webp artist-back.webp logo.png
vitest.config.ts
.env.local.example
```
...
export const metadata: Metadata = {
  ...
  description:
    "Дроп 01 — оверсайз-футболка від Саші Чемерова та гурту «Димна Суміш».",
...
- Create: `public/hero.webp`, `public/front.webp`, `public/back.webp`, `public/artist-front.webp`, `public/artist-back.webp`, `public/logo.png`
...
```bash
cp source-assets/regenerated/hero.jpeg public/hero.webp
cp source-assets/regenerated/font.jpeg public/front.webp
cp source-assets/regenerated/back.jpeg public/back.webp
cp source-assets/regenerated/artist-front.jpeg public/artist-front.webp
cp source-assets/regenerated/artist-back.jpeg public/artist-back.webp
...
export const SITE_URL =
  process.env.NEXT_PUBLIC_SITE_URL ?? "https://isusneisus.com";
...
export function Header() {
  return (
    <header className={styles.header}>
      ...
      <span className={`${styles.drop} mono`}>
        DROP 01 // ONE SIZE (OVERSIZE)
      </span>
    </header>
  );
}
  ...
  return (
    <section className={styles.hero}>
      ...
      <h1 className={`${styles.title} poster`}>
        ...
        <span>
          TOO MUCH <span className={styles.red}>ДОЛИНОЮ</span>
        </span>
      </h1>
...
.grad {
  ...
  inset: 0;
  background:
    linear-gradient(to top, rgba(0, 0, 0, 0.92) 2%, rgba(0, 0, 0, 0) 46%),
      ...
      <div className={styles.slider} aria-hidden={false}>
        <div className={styles.stage}>
          {SLIDES.map((s, i) => (
            <div
              key={s.src}
              className={`${styles.frame} ${i === active ? styles.on : ""}`}
            >
              <Image
                src={s.src}
                alt={s.cap}
...
.plate:hover .plateImg {
  opacity: 0;
}
.plate:hover .plateReveal {
  opacity: 1;
}
.tag {
  position: absolute;
  ...
  return (
    <footer className={styles.footer}>
      ...
      <nav className={styles.links}>
        <Link href="/offer" className="mono">
          Публічна оферта
        </Link>
        <Link href="/returns" className="mono">
          Умови повернення
        </Link>
      </nav>
...
export function LegalPage({
  ...
  return (
    <main className={styles.page}>
      <Link href="/" className={`${styles.back} mono`}>
        ← На головну
      </Link>
      ...
      >
        ...
        <div className={styles.scroll}>
          <div className={styles.head}>
            ...
            <button className={styles.x} onClick={close} aria-label="Закрити">
              ✕
            </button>
          </div>
...
export function CheckoutForm() {
  ...
  const set =
    (k: keyof CheckoutInput) => (e: React.ChangeEvent<HTMLInputElement>) =>
      ...
      <div className={styles.order}>
        ...
        <div className={styles.orderInfo}>
          ...
          <div className={`${styles.orderMeta} mono`}>
            OVERSIZE · ОДИН РОЗМІР · ×1
          </div>
        </div>
        <div className={`${styles.orderPrice} poster`}>
          {PRODUCT.price}&nbsp;₴
        </div>
      </div>
...
function Field(props: {
  ...
  return (
    <label className={styles.field}>
      ...
      />
      {props.hint && (
        <span className={`${styles.fieldHint} mono`}>{props.hint}</span>
      )}
    </label>
  );
}
...
export function NovaPoshtaPicker({
  ...
  return (
    <>
      ...
      <label className={styles.field}>
        <span className={`${styles.fieldLabel} mono`}>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            if (city) onSelect(city.label, city.ref, e.target.value);
          }}
        >
          <option value="">
            {city ? "Оберіть зі списку" : "Спочатку оберіть місто"}
          </option>
          {warehouses.map((w) => (
            <option key={w.ref} value={w.label}>
              {w.label}
            </option>
          ))}
        </select>
...
```tsx
<NovaPoshtaPicker
  onSelect={(city, cityRef, warehouse) =>
    setData((d) => ({ ...d, city, cityRef, warehouse }))
  }
/>
```
...
describe("callbackSignature", () => {
  it("builds signature from callback fields", () => {
    ...
    expect(sig).toBe(
      hmacMd5(SECRET, "test_merch;DROP01-1;2200;UAH;123;44**11;Approved;1100"),
    );
  });
});
...
describe("responseSignature", () => {
  it("signs orderReference;status;time", () => {
    expect(responseSignature(SECRET, "DROP01-1", "accept", 1700)).toBe(
      hmacMd5(SECRET, "DROP01-1;accept;1700"),
    );
  });
});
```
...
describe("mapCities", () => {
  it("maps NP settlement payload to options", () => {
    ...
    expect(mapCities(raw)).toEqual([
      { label: "Львів, Львівська обл.", ref: "ref-1" },
    ]);
  });
...
export function mapCities(raw: unknown): NpOption[] {
  ...
  return raw.map(
    (c: { Present?: string; Description?: string; Ref: string }) => ({
...
export function validateCheckout(input: Partial<CheckoutInput>): {
  ...
  if (!input.warehouse || !input.warehouse.trim())
    return { ok: false, reason: "warehouse" };
  return { ok: true };
}
```
...
export async function POST(req: NextRequest) {
  ...
  if (
    expected === body.merchantSignature &&
    ...
    try {
      const text = formatOrderMessage({
        orderReference: body.orderReference,
        fullName:
          `${body.clientLastName ?? ""} ${body.clientFirstName ?? ""}`.trim() ||
          "—",
